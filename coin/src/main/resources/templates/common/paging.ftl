...
<script>
		...
		$('.tablefirst').on('click',function(){
			 if(${page.first?string('true','false')}==false){
				 $('.thistable').load('${url}?page=0${(sort)!''}&type=${type}');
			 }
		});
		$('.tableup').on('click',function(){
			 if(${page.first?string('true','false')}==false){
				 $('.thistable').load('${url}?page=${(page.number)-1}${(sort)!''}&type=${type}');
			 }
		});
		...
		$('.tablelast').on('click',function(){
			if(${page.last?string('true','false')}==false){
				$('.thistable').load('${url}?page=${(page.totalPages)-1}${(sort)!''}&type=${type}');
			 }
		...
		$('.thistype').on('click', function() {
			if ($(this).children().hasClass('glyphicon-triangle-bottom')) {
				$('.thistable').load('${url}?type=0&icon=glyphicon-triangle-top${(sort2)!''}&type=${type}');
			} else {
				$('.thistable').load('${url}?type=1&icon=glyphicon-triangle-bottom${(sort2)!''}&type=${type}');
			}
		});
		$('.thisstatus').on('click', function() {
			if ($(this).children().hasClass('glyphicon-triangle-bottom')) {
				$('.thistable').load('${url}?status=0&icon=glyphicon-triangle-top${(sort2)!''}&type=${type}');
			} else {
				$('.thistable').load('${url}?status=1&icon=glyphicon-triangle-bottom${(sort2)!''}&type=${type}');
			}
		});
		$('.thistime').on('click', function() {
			if ($(this).children().hasClass('glyphicon-triangle-bottom')) {
				$('.thistable').load('${url}?time=0&icon=glyphicon-triangle-top${(sort2)!''}&type=${type}');
			} else {
				$('.thistable').load('${url}?time=1&icon=glyphicon-triangle-bottom${(sort2)!''}&type=${type}');
			}
		});
		$('.thisvisit').on('click', function() {
			if ($(this).children().hasClass('glyphicon-triangle-bottom')) {
				$('.thistable').load('${url}?visitnum=0&icon=glyphicon-triangle-top${(sort2)!''}&type=${type}');
			} else {
				$('.thistable').load('${url}?visitnum=1&icon=glyphicon-triangle-bottom${(sort2)!''}&type=${type}');
			}
		});
		/* 查找 */
